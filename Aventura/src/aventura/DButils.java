...
public class DButils 
{
    ...
    {
        // Verificar se as variÃ¡veis de instÃ¢ncia jÃ¡ contÃ©m dados
        if (vnums != null       || nomes != null        || descs != null || 
            obj_pesos != null   || sala_saidas != null  || sala_lista_obj != null)
        {
            System.out.println("DButils::carregar: !!! Esta instÃ¢ncia jÃ¡ foi carregada !!!");
            return false;
        }
        
        // Detectar formato do arquivo baseado na extensÃ£o dele
        if (nome_arq.substring(nome_arq.length()-4, nome_arq.length()).equals(".obj"))
            formato_obj = true;
                   
        // Alocar memÃ³ria para as variÃ¡veis de instÃ¢ncia
        ...
        // Verificar se arquivo existe no disco rÃ­gido
        File arquivo = new File(nome_arq);
        if (!arquivo.exists())
        {
            System.out.println("DButils::carregar: !!! Arquivo nÃ£o existe !!!");
            return false;
        }
                
        // Alocar memÃ³ria para um objeto Scanner para ler dados do arquivo
        Scanner entrada;
        try 
        {
        ...
        /* Loop que faz o parsing das informaÃ§Ãµes do arquivo
         * e armazena os dados em variÃ¡veis prÃ³prias da classe
         * para serem recuperadas depois atravÃ©s dos mÃ©todos GET.
         */
        ...
        while (entrada.hasNext()) 
        {
            ...
            // linha 3: se for sala, lista das saidas; senÃ£o, peso do objeto
            if (num_linha == 3)
            {
                if (!formato_obj)
                {
                    // Separar os nÃºmeros por espaÃ§os
    ...
    {        
        if (!formato_obj)
        {
            // Arquivos que nÃ£o sÃ£o .obj nÃ£o possuem esta informaÃ§Ã£o
            return null;
        }
